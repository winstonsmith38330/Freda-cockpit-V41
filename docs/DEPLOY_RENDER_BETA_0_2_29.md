# Deploy Freda Ops Cockpit Beta 0.2.30 on Render
...
Open `/api/diagnostics/browser-sync` and confirm:
...
Then run all stores only after one-store sync returns correctly.
